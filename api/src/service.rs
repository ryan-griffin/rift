use crate::auth::{
	AuthEvents, AuthSession, Credentials, PasswordError, generate_token, has_valid_token_shape,
	hash_password, hash_token, session_expires_at, validate_password, verify_password,
};
use crate::db;
use crate::entity::{
...
/// User details and the raw session token returned on signup or login.
#[derive(Serialize)]
...
async fn authenticate_user(
	...
	// Reject deleted accounts before checking their wiped password hashes.
...
pub async fn authenticate_token(
	...
	// Normalize header and query tokens consistently.
...
pub async fn logout_all(
	...
	delete_user_sessions(db, username).await?;
	auth_events.invalidate_user(username);
	Ok(())
}

// Database-only helper for operations that own a larger transaction.
...
/// Commit on success. On failure, log rollback errors without replacing the original error.
...
pub async fn delete_user(
	...
	let result = async {
		if let Some(user) = db::tombstone_user_by_username(&txn, authenticated_username).await? {
			delete_user_sessions(&txn, authenticated_username).await?;
			return Ok(user);
		}
	...
	let user = finish_transaction(txn, result, "delete user").await?;
	auth_events.invalidate_user(authenticated_username);
	Ok(user)
}
